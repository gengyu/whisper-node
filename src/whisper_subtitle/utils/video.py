...
class VideoDownloader:
    ...
    async def download_video(
        ...
        try:
            ...
            ydl_opts = {
                'outtmpl': str(output_path.with_suffix('')),  # 输出文件名模板（不含扩展名）
                'format': 'bestaudio/best' if audio_only else quality,  # 下载格式：音频最佳或指定视频质量
                'noplaylist': True,  # 不下载播放列表，只下载单个视频
                'extractaudio': audio_only,  # 是否仅提取音频
                'audioformat': 'mp3' if audio_only else None,  # 音频格式，仅音频下载时使用
                # 'audioquality': '192',  # 音频质量（已禁用）
                'audioquality': None,  # 使用原音频质量
                **kwargs  # 其他yt-dlp选项
            }
